...
> - **Lossless backup (2026-06-14 — follow-up done):** the round trip was lossy (`exportSettings` dumped all prefs but `importSettings` restored only 18). `SettingsBackup` is now a complete typed registry — all **57** persisted keys (bool/int/long/string), with `applyTo` reconstructing each DataStore key from name+type (equal by name+type) so there's no 57-case `when` or unchecked casts. `SettingsBackupTest` (9) now exercises the real write path via in-memory `mutablePreferencesOf` and guards the 57-key coverage. Delete-all is a confirmed destructive action in the (WIP) screen → device-verify.
