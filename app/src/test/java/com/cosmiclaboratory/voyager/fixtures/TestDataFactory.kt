...
object TestDataFactory {
    ...
    fun createVisit(
        ...
        // Compute exitTime from duration if not explicitly provided
        val resolvedExitTime = exitTime ?: entryTime.plusNanos(duration * 1_000_000)
        return Visit(
            id = id,
            placeId = placeId,
            entryTime = entryTime,
    ...
    fun generateVisitsForAnalytics(
        ...
        return (0 until count).map { i ->
            val dayOffset = i * (30 / count)
            val entryTime = startDate.plusDays(dayOffset.toLong()).plusHours(9)
            val duration = (3600000L + (Math.random() * 7200000).toLong())
