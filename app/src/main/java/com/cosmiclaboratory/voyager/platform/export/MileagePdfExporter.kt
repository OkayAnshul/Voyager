package com.cosmiclaboratory.voyager.platform.export

import android.content.Context
import android.graphics.Paint
import android.graphics.pdf.PdfDocument
import android.net.Uri
import androidx.core.content.FileProvider
import com.cosmiclaboratory.voyager.domain.model.MileageLog
import com.cosmiclaboratory.voyager.domain.model.MileageRateConfig
import com.cosmiclaboratory.voyager.domain.model.MileageRatePreset
import com.cosmiclaboratory.voyager.domain.model.formatMoney
import com.cosmiclaboratory.voyager.domain.model.shortLabel
import com.cosmiclaboratory.voyager.domain.usecase.MileageDeduction
import dagger.hilt.android.qualifiers.ApplicationContext
import java.io.File
import java.time.Instant
import java.time.ZoneId
import java.time.format.DateTimeFormatter
import javax.inject.Inject
import javax.inject.Singleton

/**
 * Renders a [MileageLog] to a tax-ready PDF using the platform [PdfDocument].
 *
 * Layout is US Letter (612×792 pt): a per-purpose summary with an estimated deduction,
 * then a paginated table of every drive. Rates, currency, and distance unit all come from
 * the user's [MileageRateConfig], and the figure is clearly labelled an estimate the filer
 * must verify — rates change yearly and jurisdictions differ.
 */
@Singleton
class MileagePdfExporter @Inject constructor(
    @ApplicationContext private val context: Context
) {

    private companion object {
        const val PAGE_WIDTH = 612
        const val PAGE_HEIGHT = 792
        const val MARGIN = 48f
        const val LINE = 16f

        val DATE_FMT: DateTimeFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd")
        val TIME_FMT: DateTimeFormatter = DateTimeFormatter.ofPattern("HH:mm")
    }

    private val zone: ZoneId get() = ZoneId.systemDefault()

    /** Writes [log] to a cached PDF and returns a shareable [Uri], using [config] for rates/units. */
    fun export(log: MileageLog, config: MileageRateConfig): Result<Uri> = runCatching {
        val doc = PdfDocument()
        try {
            renderInto(doc, log, config)
        } catch (e: Exception) {
            doc.close()
            throw e
        }

        val file = File(context.cacheDir, "voyager_mileage_${System.currentTimeMillis()}.pdf")
        file.outputStream().use { doc.writeTo(it) }
        doc.close()

        FileProvider.getUriForFile(context, "${context.packageName}.fileprovider", file)
    }

    private fun renderInto(doc: PdfDocument, log: MileageLog, config: MileageRateConfig) {
        val title = Paint().apply { textSize = 20f; isFakeBoldText = true }
        val heading = Paint().apply { textSize = 12f; isFakeBoldText = true }
        val body = Paint().apply { textSize = 10f }
        val muted = Paint().apply { textSize = 9f; color = 0xFF666666.toInt() }

        val unit = config.distanceUnit
        val unitLabel = unit.shortLabel
        val estimate = MileageDeduction.estimate(log, config)
        val currency = estimate.currencyCode

        var pageNum = 1
        var page = doc.startPage(PdfDocument.PageInfo.Builder(PAGE_WIDTH, PAGE_HEIGHT, pageNum).create())
        var canvas = page.canvas
        var y = MARGIN

        // ── Title block ───────────────────────────────────────────────
        canvas.drawText("Mileage Log", MARGIN, y, title); y += LINE * 1.5f
        canvas.drawText("Period: ${log.rangeLabel}", MARGIN, y, body); y += LINE
        canvas.drawText(
            "Generated: ${DATE_FMT.format(Instant.now().atZone(zone))}",
            MARGIN, y, muted
        ); y += LINE * 1.6f

        // ── Summary ───────────────────────────────────────────────────
        canvas.drawText("Summary", MARGIN, y, heading); y += LINE * 1.2f
        canvas.drawText("Purpose", MARGIN, y, muted)
        canvas.drawText("Distance ($unitLabel)", MARGIN + 220f, y, muted)
        canvas.drawText("Est. amount ($currency)", MARGIN + 320f, y, muted); y += LINE

        for (line in estimate.lines) {
            canvas.drawText(line.purpose.displayName, MARGIN, y, body)
            canvas.drawText(fmt1(line.distance), MARGIN + 220f, y, body)
            canvas.drawText(
                if (line.rate != null) formatMoney(line.amount, currency) else "—",
                MARGIN + 320f, y, body
            )
            y += LINE
        }
        y += LINE * 0.4f
        canvas.drawText("Total distance", MARGIN, y, heading)
        canvas.drawText(fmt1(estimate.totalDistance), MARGIN + 220f, y, heading)
        canvas.drawText(formatMoney(estimate.totalAmount, currency), MARGIN + 320f, y, heading); y += LINE * 1.4f

        for (disclaimerLine in disclaimerLines(config.ratePreset)) {
            canvas.drawText(disclaimerLine, MARGIN, y, muted); y += LINE * 0.9f
        }
        y += LINE * 1.1f

        // ── Detail table ──────────────────────────────────────────────
        canvas.drawText("Drives", MARGIN, y, heading); y += LINE * 1.2f
        fun drawColumnHeaders() {
            canvas.drawText("Date", MARGIN, y, muted)
            canvas.drawText("Time", MARGIN + 90f, y, muted)
            canvas.drawText(unitLabel, MARGIN + 150f, y, muted)
            canvas.drawText("Purpose", MARGIN + 220f, y, muted)
            canvas.drawText("Note", MARGIN + 320f, y, muted)
            y += LINE
        }
        drawColumnHeaders()

        for (entry in log.entries) {
            if (y > PAGE_HEIGHT - MARGIN) {
                doc.finishPage(page)
                pageNum += 1
                page = doc.startPage(
                    PdfDocument.PageInfo.Builder(PAGE_WIDTH, PAGE_HEIGHT, pageNum).create()
                )
                canvas = page.canvas
                y = MARGIN
                drawColumnHeaders()
            }
            val start = Instant.ofEpochMilli(entry.startAt).atZone(zone)
            canvas.drawText(DATE_FMT.format(start), MARGIN, y, body)
            canvas.drawText(TIME_FMT.format(start), MARGIN + 90f, y, body)
            canvas.drawText(fmt1(entry.distanceIn(unit)), MARGIN + 150f, y, body)
            canvas.drawText(entry.purpose.displayName, MARGIN + 220f, y, body)
            canvas.drawText(truncateNote(entry.note), MARGIN + 320f, y, body)
            y += LINE
        }

        if (log.entries.isEmpty()) {
            canvas.drawText("No drives recorded in this period.", MARGIN, y, body)
        }

        doc.finishPage(page)
    }

    /** Preset-specific disclaimer text — never presents a figure as anything but an estimate. */
    private fun disclaimerLines(preset: MileageRatePreset): List<String> = when (preset) {
        MileageRatePreset.IRS_2025 -> listOf(
            "Estimate uses IRS 2025 standard mileage rates. Verify current rates for your",
            "tax year and jurisdiction before filing. Personal mileage is not deductible."
        )
        MileageRatePreset.HMRC -> listOf(
            "Estimate uses HMRC approved mileage rates, simplified to a flat 45p/mile. The",
            "scheme drops to 25p after 10,000 business miles/year — use a custom rate past that.",
            "Verify current rates before filing. Personal mileage is not deductible."
        )
        MileageRatePreset.CUSTOM -> listOf(
            "Estimate uses your custom mileage rates. Verify they match your jurisdiction or",
            "reimbursement policy before filing. Personal mileage is not deductible."
        )
    }

    private fun fmt1(value: Double): String = "%.1f".format(value)

    private fun truncateNote(note: String?): String {
        val n = note?.trim().orEmpty()
        return if (n.length > 32) n.take(31) + "…" else n
    }
}
