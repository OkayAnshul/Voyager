...
object LocationUtils {
    ...
    private fun expandCluster(
        ...
        val clusterIndices = mutableSetOf(pointIndex)
        val neighborSet = neighbors.toMutableSet()
        ...
        while (i < neighbors.size) {
            ...
            if (!visited[neighborIndex]) {
                ...
                if (neighborNeighbors.size + 1 >= minPoints) {
                    for (newNeighbor in neighborNeighbors) {
                        if (newNeighbor !in neighborSet) {
                            neighborSet.add(newNeighbor)
            ...
            // Add point to cluster if not already added (border points allowed)
            if (neighborIndex !in clusterIndices) {
                clusterIndices.add(neighborIndex)
                noise[neighborIndex] = false
