...
class OverpassApiServiceImpl @Inject constructor(
    ...
    override suspend fun findNearbyPoi(
        ...
        try {
            ...
            // Find closest POI with a valid name and resolved coordinates
            val closestPoi = overpassResponse.elements
                .filter { it.tags?.name != null && !isGenericName(it.tags.name) && it.resolvedLat != null && it.resolvedLon != null }
                .minByOrNull { element ->
                    calculateDistance(latitude, longitude, element.resolvedLat!!, element.resolvedLon!!)
                }
            ...
            val poiLat = closestPoi.resolvedLat!!
            val poiLon = closestPoi.resolvedLon!!
...
private data class Element(
    ...
    val lat: Double? = null,
    val lon: Double? = null,
    val center: ElementCenter? = null,
    val tags: Tags? = null
) {
    /** Resolved lat/lon — nodes have direct coords, ways/relations use center */
    val resolvedLat: Double? get() = lat ?: center?.lat
    val resolvedLon: Double? get() = lon ?: center?.lon
}

@Serializable
private data class ElementCenter(
    val lat: Double,
    val lon: Double
)
