...
data class Visit(
    ...
    // Duration calculation: exitTime is authoritative, then stored, then live
    val duration: Long
        get() = when {
            exitTime != null -> java.time.Duration.between(entryTime, exitTime).toMillis()
            _duration > 0L -> _duration
            else -> java.time.Duration.between(entryTime, LocalDateTime.now()).toMillis()
        }
